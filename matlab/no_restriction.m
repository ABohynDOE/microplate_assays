% BASED ON NO_RESTRICTION
clear; clc;
delete no_restriction.txt;
diary no_restriction.txt;
%% Build base design
design = zeros(32,8);
...
% Factor f = abcd
design(:,6) = prod(design(:,[1,2,3,4]),2);
% Factor g = abe
design(:,7) = prod(design(:,[1,2,5]),2);
% Factor h = ace
design(:,8) = prod(design(:,[1,3,5]),2);
...
pf = zeros(32,7);
% p1 = abc
pf(:,1) = prod(design(:,[1,2,3]),2);
% p2 = ad
pf(:,2) = prod(design(:,[1,4]),2);
% p3 = ae
pf(:,3) = prod(design(:,[1,5]),2);
% p4 = p1 p2
pf(:,4) = prod(pf(:,[1,2]),2);
% p5 = p1 p3
pf(:,5) = prod(pf(:,[1,3]),2);
...
% There are 8 + 28 + 56 = 92 factorial effects + 7 PF effects and 31
% degrees of freedom so there will be 99-31=68 alias covered
...
% Week is defined as b2=abd
week = (prod(design(:,[1,2,4]), 2)+1)./2;
% Plate is defined as b1=ade * b2=abd
plate = prod(design(:,[1,4,5]), 2)+1+(prod(design(:,[1,2,4]), 2)+1)./2;
% Tube is defined as b2=abd*a*b*c
tube = (prod(design(:,[1,2,4]), 2)+1).*4 +(design(:,1)+1).*2 + (design(:,2)+1) + (design(:,3)+1)./2;
