...
clear; clc;
delete tubes_twice_per_plate.txt;
diary tubes_twice_per_plate.txt;
%% Build base design
design = zeros(32,8);
...
% Plate is defined as b1=ace * b2=abc
plate = prod(design(:,[1,3,5]), 2)+1+(prod(design(:,[1,2,3]), 2)+1)./2;
% Tube is defined as b2=abc*a*b*e
tube = (prod(design(:,[1,2,3]), 2)+1).*4 +(design(:,1)+1).*2 + (design(:,2)+1) + (design(:,5)+1)./2;
